    ...
    public boolean action(String message, String sender, String channel, GroupTracker bot) {
        ...
        if (s != null) {
            ...
            if (s != null) {
                ...
                s = bot.containsPhrase(message, GroupType.getGroupWords());
                if (s != null) {
                    String groupType = s.get(0);
                    System.out.println("String gotten: " + groupType);
                    ...
                    for (Modifier m : g.modifiers) {
                        ...
                        if (!m.requiredMods.isEmpty()) {
                            for (Modifier m2 : g.modifiers) {
                                boolean contains = false;
                                for (String s2 : m.requiredMods) {
                                    if (m2.searchText.contains(s2)) {
                                        deleteMod = false;
                                        contains = true;
                                        break;
                                    }
