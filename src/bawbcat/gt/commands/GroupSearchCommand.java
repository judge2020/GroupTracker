...
public class GroupSearchCommand extends Command {
    ...
    public boolean action(String message, String sender, String channel, GroupTracker bot) {
        List<String> s = bot.containsPhrase(message, GroupType.getGroupWords());
        List<Group> searchResults = new ArrayList<Group>(bot.groups);
        boolean allGroups = true;
        if (s != null || bot.contains(message, "all") || bot.contains(message, "any") || bot.containsPhrase(message, bot.districts, true) != null || bot.containsPhrase(message, GroupType.getAllModifiers().toArray(), true) != null || bot.containsPhrase(message, Playground.getAllSearchText().toArray()) != null || bot.containsPhrase(message, Playground.getAllStreets().toArray(), true) != null) {
            ...
            if (s != null) {
                ...
                for (Modifier[] array : searchType.validModifiers) {
            ...
            if (!searchResults.isEmpty()) {
                ...
                if (searchSize > antiSpamLimit) { //AntiSpam search
                    response = Colors.DARK_BLUE + sender + Colors.NORMAL + ", there ";
                    ...
                    while (!searchResults.isEmpty()) {                    
                        ...
                        if (typeAmounts.get(g.type) > 1 && allGroups) {
                            response = response + typeAmounts.get(g.type) + " " + Colors.PURPLE + Colors.BOLD + g.type.display + Colors.NORMAL + " groups";
                            gaveGroupAmount = true;
                        } else {
                            response = response + "a " + g.getTypeString() + " group owned by " + Colors.BROWN + Colors.BOLD + g.owner + Colors.NORMAL;
                            gaveGroupOwner = true;
                        }
                               
                        if (searchResults.get(searchResults.size() - 1) == g) {
                            response = response + ". For more information, say" + (gaveGroupAmount ? " '" + Colors.BOLD + "<group type> groups" + Colors.NORMAL + "'" : "") + (gaveGroupAmount && !gaveGroupOwner ? "." : " ") + (gaveGroupAmount && gaveGroupOwner ? "or " : "") + (gaveGroupOwner ? "'" + Colors.BOLD + "<group owner> group info" + Colors.NORMAL + "'." : "");
                    ...
                    response = Colors.DARK_BLUE + sender + Colors.NORMAL + ", there is ";
                    while (!searchResults.isEmpty()) {
                        Group g = searchResults.get(0);
                ...
                if (searchType != null) {
                    ...
                    for (Modifier[] mArray : searchType.validModifiers) {
                    ...
                    bot.sendMessage(channel, Colors.DARK_BLUE + sender + Colors.NORMAL + ", there aren't any " + fakeGroup.getTypeString() + " groups currently.");
                } else {
                    bot.sendMessage(channel, Colors.DARK_BLUE + sender + Colors.NORMAL + ", there aren't any groups currently.");
                }
            }
            return true;
        }
