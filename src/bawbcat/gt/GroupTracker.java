...
public class GroupTracker extends OromusBot implements ActionListener {
    ...
    public String[] joinChannels = {"#toontownfellowship", "#ttfgrouptrackertest"};
    //public String[] joinChannels = {"#grouptrackertest"};
    
    public String[] districts = {"Cog River", "Hollandia", "Nutty Glacier", "Nutty Kitty", "Peaceful Peaks", "Phoenix Rising"};

    public String[] presetOps = {"Judge", "Ditto", "BawbCat"};
    ...
    public GroupTracker() {
        ...
        GroupType.make("Bean Fest", new String[]{"beanfest", "bean", "fest"}).needsPlayground(true).maxToons(999);
        
        GroupType.make("Toon Tasking", new String[]{"tasking", "task", "toontask"}).needsPlayground(true);
        
        GroupType.make("Gag Training", new String[]{"training"}).needsPlayground(true)
                .addMods(new Modifier[]{new Modifier("Toonup", false), new Modifier("Trap", false), new Modifier("Lure", false), new Modifier("Sound", false), new Modifier("Throw", false), new Modifier("Squirt", false), new Modifier("Drop", false)});
              
        GroupType.make("building", new String[]{"build", "bldg", "bld"}).needsStreet(true)
                .addMods(new Modifier[]{new Modifier("1 story", new String[]{"1", "one"}), new Modifier("2 story", new String[]{"2", "two"}), new Modifier("3 story", new String[]{"3", "three"}), new Modifier("4 story", new String[]{"4", "four"}), new Modifier("5 story", new String[]{"5", "five"})})
                .addMods(new Modifier[]{new Modifier("Sellbot", new String[]{"Sell"}), new Modifier("Cashbot", new String[]{"Cash"}), new Modifier("Lawbot", new String[]{"Law"}), new Modifier("Bossbot")});
            
        GroupType.make("Field Office", new String[]{"fieldoffice"}).needsStreet(true)
                .addMods(new Modifier[]{new Modifier("Sellbot", new String[]{"Sell"}), new Modifier("Cashbot", new String[]{"Cash"}), new Modifier("Lawbot", new String[]{"Law"}), new Modifier("Bossbot")});
        
        GroupType.make("Factory", new String[]{"fact"})
                .addMods(new Modifier[]{new Modifier("Front", false), new Modifier("Side", false), new Modifier("Brutal", false)})
                .addMods(new Modifier[]{new Modifier("Short", false), new Modifier("Long", false), new Modifier("Hybrid", false)});
               
        GroupType.make("Mint")
                .addMods(new Modifier[]{new Modifier("Coin", false), new Modifier("Dollar", false), new Modifier("Bullion", false)});
                
        GroupType.make("D.A. Office", new String[]{"da office", "daoffice", "da"})
                .addMods(new Modifier[]{new Modifier("A", false).pos(ModifierPos.BACK), new Modifier("B", false).pos(ModifierPos.BACK), new Modifier("C", false).pos(ModifierPos.BACK), new Modifier("D", false).pos(ModifierPos.BACK)});
             
        GroupType.make("Cog Golf", new String[]{"front", "middle", "back"}).hideNameIfHasMods(true)
                .addMods(new Modifier[]{new Modifier("Front Three", new String[]{"Front"}), new Modifier("Middle Six", new String[]{"Middle"}), new Modifier("Back Nine", new String[]{"Back"})});
        
        GroupType.make("boss", new String[]{"VP", "CFO", "CJ", "CEO", "sos", "shopping"}).hideNameIfHasMods(true)
                .addMods(new Modifier[]{new Modifier("SOS Shopping", false, new String[]{"sos", "shopping"}).requires("VP")})
                .addMods(new Modifier[]{new Modifier("Brutal", false, new String[]{"Brutal"}).requires("VP").requires("CFO")})
                .addMods(new Modifier[]{new Modifier("VP"), new Modifier("CFO"), new Modifier("CJ"), new Modifier("CEO")});
    ...
    protected void onConnect() {
        //sendRawLine("AUTHSERV COOKIE GroupTracker IKqoauMzYE");
        sendRawLine("AUTHSERV AUTH ExtrasBot fnabCelI");
    ...
    public void process(String message, String sender, String channel) {
        ...
        removedGroups.clear();
         ...
         if (enabled) {
            ...
            if (contains(message, new String[]{"GS", "GT"}) != null || contains(message, getName()) || sender.equals(channel)) {
            ...
            if (contains(message, new String[]{"GS", "GT"}) != null || contains(message, getName()) || sender.equals(channel)) {
                if (contains(message, "enable") || contains(message, "toggle")) {
                    if (ops.contains(sender)) {
                        enabled = true;
                        sendNotice(sender, Colors.PURPLE + Colors.BOLD + getName() + Colors.NORMAL + " is now " + Colors.DARK_GREEN + Colors.BOLD + "enabled" + Colors.NORMAL + ".");
                    }
                }
            }
        }
            ...
            if (groupFile.exists()) {
                ...
                for (String s : groupStrings) {
                    ...
                    for (int i = 6; i < info.length; i++) {
                        for (Modifier[] array : g.type.validModifiers) {
